...
- 启动、停止和查看服务器状态
- 在浏览器中使用服务端控制台
- 可视化管理连接、网络、运行、封禁列表和模组路径等服务端配置
- 识别 SpamCheck/`Projectile spam`，检查 `secure` 与启动脚本冲突并明确关闭
- 上传、启用、停用和删除本地 `.tmod` 模组
- 查看已安装的 Workshop 模组
- 上传、切换和删除世界存档
- 浏览服务器目录，上传、下载、编辑文本、移动、重命名和删除文件
- 预览并安全解压 ZIP 模组整合包
- 创建、下载、恢复和删除备份
...
“服务器”页面可直接配置玩家上限、端口、密码、欢迎消息、语言、`secure`、UPnP、NPC 同步频率、进程优先级、封禁列表路径、本地模组目录和 Mod Pack。世界路径、世界大小、难度和种子继续在“存档”页面管理；当前版本尚未覆盖或未来新增的 tModLoader 字段仍可通过“打开配置文件”直接编辑。

“服务器”页面会检查当前世界配置中的 `secure`、启动脚本中的活动 `-secure` 参数，以及近期服务端日志中的 `Projectile spam`。需要排查大型 Mod 武器误触发时，先停止服务端，再使用“明确关闭 SpamCheck”写入 `secure=0`；也可以直接打开当前世界配置或启动脚本核对原文。

关闭 `secure` 会降低服务端原版反作弊能力，公开服务器不建议长期关闭。配置修改后需要完全重启 tModLoader 才会生效；如果启动脚本仍含 `-secure`，面板会继续显示冲突而不会自动改写脚本。

### 文件与整合包

“文件”页面管理当前服务器实例的保存根目录，可以进入目录、创建目录、上传和下载文件，以及编辑文本、移动、重命名和删除文件。受支持的小型 UTF-8 配置与脚本可以直接打开；保存时会检查文件是否在打开后被其他操作修改。服务器运行期间保持只读；上传、编辑、解压、移动或删除前需要先停止服务端。
